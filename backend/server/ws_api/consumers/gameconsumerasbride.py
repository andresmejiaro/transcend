...
class GameConsumerAsBridge(AsyncWebsocketConsumer):
    ...
    def finalize_match(self):
        ...
        try:
            self.run_game = False
            print(f'Finalizing match {self.match_id}')
            ...
            print(f'Error during match finalization: {e}')
    ...
    async def initialize_game(self):
        try:
            # Create the game
            game = Game(
                dictKeyboard=self.keyboard_inputs,  # This is a dprinttionary that holds the keyboard inputs for each match (eg. self.keyboard_inputs = {"up.1": False, "down.1": False, "up.2": False, "down.2": False})
    ...
    async def disconnect(self, close_code):
        try:
            ...
            # await self.get_match(self.match_id)
            print(f'Disconnecting from match {self.match_id} with client {self.client_id}. Player 1: {self.player_1_id}. Player 2: {self.player_2_id}')
            if self.client_id in self.list_of_players:
                print(f'Stopping game {self.match_id}')
                if self.list_of_games.get(self.match_id) and self.list_of_games[self.match_id].isAlive():
                    print(f'Sending stop command to game {self.match_id}')
            ...
            print("Disconnect operation timed out. Closing the connection.")
        except Exception as e:
    ...
    async def receive(self, text_data):
        ...
        # form of a JSON string. We parse the JSON string into a dprinttionary and extract the command and data from the dprinttionary.
