...
class Tournament2Consumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.list_of_player_channels = Tournament2Consumer.list_of_player_channels[self.tournament_id]
        list_of_current_players = list(self.list_of_player_channels.keys())

        if len(list_of_current_players) == 1:
            self.tournament_admin_id = self.player_object
            Tournament2Consumer.tournament_objects[self.tournament_id] = await self.get_tournament_object()
    ...
    def init_player_obj(self, pk):
        try:
            ...
            if not Tournament2Consumer.list_of_player_channels[self.tournament_id]:
                Tournament2Consumer.list_of_player_channels[self.tournament_id] = dict()
            Tournament2Consumer.list_of_player_channels[self.tournament_id][self.client_id] = self.channel_name
            return user
        except Exception as e:
            print(e)
            return None
