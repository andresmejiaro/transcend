...
# Preview of the Profile view
    def display(self):
        try:
            ...
            user_friends = self.http.get_list_friends()
            log_message(f"User friends: {user_friends}", level=logging.INFO)
            if user_friends and user_friends.get("status") == "ok":
                self.stdscr.addstr(2, 1, "Friends: ")
                if user_friends["data"]:
                    for i, friend_data in enumerate(user_friends["data"]):
                        username = friend_data.get("username", "")
                        # Adjust the row position and add some space between usernames
                        self.stdscr.addstr(3 + i, 1, f"{i + 1}. {username}")
                else:
                    self.stdscr.addstr(3, 1, "No friends found.")
            else:
                self.stdscr.addstr(3, 1, "Error fetching friends data.")

        except Exception as e:
            log_message(f"Error displaying friends: {e}", level=logging.ERROR)
